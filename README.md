...
如果需要跑完整 TypeScript 严格质量门，额外安装 OpenClaw plugin 依赖：

```bash
cd extensions/agora-plugin && npm install
```
...
- 从项目根目录 `.env` 读取统一开发配置
- 启动 `agora-ts` Fastify Server
- 启动 `dashboard` Vite Dev Server
- 自动处理 `agora-ts` / `dashboard` 的本地依赖缺失

常用地址：

- `http://127.0.0.1:18420/api/health`
- `http://127.0.0.1:33173/dashboard/`
- `http://127.0.0.1:18420/dashboard/`（生产构建后由 server 静态挂载）
...
```bash
cd agora-ts
npm run dev -w @agora-ts/cli -- craftsman tmux up
npm run dev -w @agora-ts/cli -- craftsman tmux doctor
npm run dev -w @agora-ts/cli -- craftsman history <taskId> <subtaskId>
```

### Create a Task

Discord 侧仍可通过 Slash Command 创建：
...
- [docs/Agora-实施排期-Agora-TS.md](/Users/lizeyu/Projects/Agora/docs/Agora-实施排期-Agora-TS.md)
