...
```bash
pip install agora-orchestrator
agora init --adapter openclaw
agora run
```
...
```
agora/
├── core/           任务管理、状态机、Gate、调度、归档
├── adapters/       平台与 Agent 框架适配（OpenClaw、Slack、...）
├── craftsmen/      执行工具适配（Claude Code、Codex、Gemini CLI、...）
├── dashboard/      可视化面板（React + Zustand）
└── templates/      治理预设与任务模板
```
