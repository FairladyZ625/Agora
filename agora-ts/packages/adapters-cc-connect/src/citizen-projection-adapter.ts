...
import type { CitizenProjectionPort } from '@agora-ts/core';
...
export class CcConnectCitizenProjectionAdapter implements CitizenProjectionPort {
  readonly adapter = 'cc-connect';
  ...
  renderPreview(input: Parameters<CitizenProjectionPort['renderPreview']>[0]): CitizenProjectionPreviewDto {
    ...
    const scaffoldMarkdown = [
      ...
      toMarkdownSection('Bridge Notes', [
        '- This preview targets an externally managed cc-connect bridge.',
        '- Agora treats cc-connect runtime state as external adapter state, not Core truth.',
        `- auto_provision: ${input.citizen.runtime_projection.auto_provision ? 'true' : 'false'}`,
      ]),
      ...(scaffold ? [
    ...
    return {
      ...
      summary: `cc-connect preview for ${input.citizen.display_name} in project ${input.project.id}`,
      files: [
        {
          path: `.cc-connect/citizens/${input.citizen.citizen_id}/profile.json`,
          content: `${JSON.stringify(profile, null, 2)}\n`,
        },
        {
          path: `.cc-connect/citizens/${input.citizen.citizen_id}/brain/03-citizen-scaffold.md`,
