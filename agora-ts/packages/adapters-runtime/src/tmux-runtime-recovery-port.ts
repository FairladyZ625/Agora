...
import type { InteractiveRuntimePort, RuntimeRecoveryPort } from '@agora-ts/core';

const TMUX_AGENTS = new Set(['codex', 'claude', 'gemini']);

export class TmuxRuntimeRecoveryPort implements RuntimeRecoveryPort {
  constructor(private readonly runtime: Pick<InteractiveRuntimePort, 'doctor' | 'tail' | 'sendKeys'>) {}
