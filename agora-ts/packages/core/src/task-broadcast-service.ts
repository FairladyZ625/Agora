    ...
    for (const member of task.team.members.filter((candidate) => (
      ...
      messages.push({
        ...
        body: [
          ...
          ...(member.runtime_target_ref ? [`${taskText(task, 'Runtime Target', 'Runtime Target')}: ${member.runtime_target_ref}`] : []),
          ...(member.runtime_flavor ? [`${taskText(task, 'Runtime Flavor', 'Runtime Flavor')}: ${member.runtime_flavor}`] : []),
          ...(member.runtime_selection_source ? [`${taskText(task, '选择来源', 'Selection Source')}: ${member.runtime_selection_source}`] : []),
          ...(member.runtime_selection_reason ? [`${taskText(task, '选择原因', 'Selection Reason')}: ${member.runtime_selection_reason}`] : []),
          `${taskText(task, '主控', 'Controller')}: ${controllerRef ?? '-'}`,
          `${taskText(task, '当前阶段', 'Current Stage')}: ${task.current_stage}`,
          `${taskText(task, '任务目标', 'Task Goal')}: ${task.description?.trim() || task.title}`,
