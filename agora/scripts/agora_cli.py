...
def list_tasks(
    state: str = typer.Option(None, "--state", "-s", help="按状态筛选: draft/created/active/blocked/paused/done/cancelled/orphaned"),
):
    """列出任务。"""
    mgr = get_manager()
...
@app.command()
def cleanup(
    task_id: str = typer.Argument(None, help="指定任务 ID，不指定则清理所有 orphaned"),
