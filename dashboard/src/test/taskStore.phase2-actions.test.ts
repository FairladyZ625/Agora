...
describe('task store phase 2 actions', () => {
  beforeEach(() => {
    ...
    vi.mocked(api.getHealthSnapshot).mockResolvedValue({
      generated_at: '2026-03-07T01:00:00.000Z',
      tasks: { status: 'healthy', total_tasks: 1, active_tasks: 1, paused_tasks: 0, blocked_tasks: 0, done_tasks: 0 },
      im: { status: 'healthy', active_bindings: 1, active_threads: 1, bindings_by_provider: [{ label: 'discord', count: 1 }] },
      runtime: { status: 'healthy', available: true, stale_after_ms: 300000, active_sessions: 1, idle_sessions: 0, closed_sessions: 0, agents: [] },
      craftsman: { status: 'healthy', active_executions: 0, queued_executions: 0, running_executions: 0, waiting_input_executions: 0, awaiting_choice_executions: 0, active_by_assignee: [] },
      host: { status: 'healthy', snapshot: null },
      escalation: {
        status: 'healthy',
        policy: { controller_after_ms: 600000, roster_after_ms: 1200000, inbox_after_ms: 1800000 },
